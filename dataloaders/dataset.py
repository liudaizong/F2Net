...
class PairwiseImg(Dataset):
        ...
        if self.seq_name is None: #所有的数据集都参与训练
            with open(os.path.join(db_root_dir, fname + '.txt')) as f:
                ...
                # #data_list = np.sort(os.listdir(db_root_dir))
                    ...
                    #     seq = seq.strip('\n') 
                if self.train:
                    images = np.sort(os.listdir(os.path.join(img_root_dir,'Imgs')))#针对某个数据集，比如DUT
                    images_path = list(map(lambda x: os.path.join(('Imgs'), x), images))         
                    image_list.extend(images_path)
                    lab = np.sort(os.listdir(os.path.join(img_root_dir,'GTs')))
                    lab_path = list(map(lambda x: os.path.join(('GTs'),x), lab))
    ...
    def __getitem__(self, idx):
        target, target_gt, target_guass = self.make_video_gt_pair(idx)
        target_id = idx
        seq_name1 = self.video_list[idx].split('/')[-2] #获取视频名称
        if self.train:
            my_index = self.Index[seq_name1]
            search_id = my_index[0]
            ...
            search, search_gt, search_guass = self.make_video_gt_pair(search_id)
            if search_id == target_id:
                previous_id = target_id
            else:
                previous_id = target_id - 1
            previous, previous_gt, previous_guass = self.make_video_gt_pair(previous_id)
            img_idx = np.random.randint(1,len(self.image_list)-1)
            img, img_gt, img_guass = self.make_img_gt_pair(img_idx)
            target_ct = centerpoint(target_gt)
            previous_ct = centerpoint(previous_gt)
            img_ct = centerpoint(img_gt)
            sample = {'target': target, 'target_gt': target_gt, 'target_guass': target_guass, 'search': search, 'search_gt': search_gt, 'search_guass': search_guass, \
                      'previous': previous, 'previous_gt': previous_gt, 'previous_guass': previous_guass, 'img': img, 'img_gt': img_gt, 'img_guass': img_guass, \
                      'target_ct': target_ct, 'previous_ct': previous_ct, 'img_ct': img_ct}
            ...
            # if search_id == target_id:
            #     search_id = np.random.randint(my_index[0], my_index[1])
            search, search_gt, _ = self.make_video_gt_pair(search_id)
            if search_id == target_id:
                previous_id = target_id
            else:
                previous_id = target_id - 1
            ...
            sample = {'target': target, 'target_gt': target_gt, 'previous': previous, 'previous_guass': previous_guass, 'search': search, 'search_gt': search_gt, 'seq_name': seq_name1, \
                        'target_ct': target_ct}
            if self.seq_name is not None:
                fname = os.path.join(self.seq_name, "%05d" % idx)
                sample['fname'] = fname
    ...
    def make_video_gt_pair(self, idx): #这个函数存在的意义是为了getitem函数服务的
        ...
        if self.labels[idx] is not None and self.train:
            ...
            gt = cv2.imread(os.path.join(self.db_root_dir, self.labels[idx]), cv2.IMREAD_GRAYSCALE)
...
if __name__ == '__main__':
    ...
    mask_path = '/Users/liudaizong/Downloads/DAVIS2016/Annotations/480p/camel/00046.png'
    mask = cv2.imread(mask_path, cv2.IMREAD_GRAYSCALE)
    # mask = imresize(mask, (473, 473), interp='nearest')
    mask[mask!=0] = 1
    bbox = extract_bboxes(mask)

    # def generate_gaussian_map(bbox, img_size, missed_frame_num=0, max_num=None):
    #     """
    #     bbox: [x1, y1, x2, y2]
    #     img_size: [h,w]
    ...
    #     y = y - center_y
    #     if max_num:
    #         sigma_x = (bbox[2]-bbox[0])*(1.05 ** min(max_num,missed_frame_num)) / 2.0

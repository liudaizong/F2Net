...
from dataloaders import dataset as db
...
from deeplab.anchor import CoattentionNet
from deeplab.decode import hoidet_decode
from deeplab.motion_model import motion_model_ct
from dataloaders.dataset import guassian
...
def configure_dataset_model(args):
    if args.dataset == 'voc12':
        ...
        args.data_list = '/mnt/cephfs_new_wj/vc/ldz/data/DAVIS2016/val_seqs.txt'  # Path to the file listing the images in the dataset
        args.ignore_label = 255     #The index of the label to ignore during the training
        ...
        args.restore_from = './snapshots/davis_iteration_conf_bad/co_attention_davis_1.pth' #resnet50-19c8e357.pth''/home/xiankai/PSPNet_PyTorch/snapshots/davis/psp_davis_0.pth' #
...
def main():
    ...
    for index, batch in enumerate(testloader):
        ...
        target = batch['target']
        target_ct = batch['target_ct']
        print('ground truth ct: ', target_ct//4+1)
        #search = batch['search']
        temp = batch['seq_name']
        ...
        #output ct
        if my_index == 0:
            previous_gauss = torch.zeros(1, 1, 119, 119).float()
            # previous_gauss = batch['previous_guass']
        previous_gauss = batch['previous_guass']
        output = model(Variable(search, volatile=True).cuda(), Variable(previous, volatile=True).cuda(), Variable(target, volatile=True).cuda(), Variable(target_ct.float(), volatile=True).cuda(), Variable(previous_gauss, volatile=True).cuda())
        output_ct = output[-1].sigmoid_()
        score, x, y = hoidet_decode(output_ct)
        print(score)
        print(x)
        print(y)

        #plot output_ct
        # ct_plot = output_ct.view(119, 119)
        # ct_plot = ct_plot.cpu().detach().numpy()
        # first_image = np.array(Image.open(args.data_dir+'/JPEGImages/480p/blackswan/00000.jpg'))
        # original_shape = first_image.shape 
        # ct_plot = cv2.resize(ct_plot, (original_shape[1],original_shape[0]))
        # save_dir_res = os.path.join(args.seg_save_dir, 'cts', args.seq_name)
        # if not os.path.exists(save_dir_res):
        #     os.makedirs(save_dir_res)
        # if args.save_segimage:   
        #     my_index1 = str(my_index).zfill(5)
        #     seg_filename = os.path.join(save_dir_res, '{}.png'.format(my_index1))
        #     ct_plot = (ct_plot*255).astype(np.uint8)
        #     ct_plot = Image.fromarray(ct_plot)
        #     ct_plot.save(seg_filename)
        
        #plot heat
        # ct_plot = output[-1]
        # heatmap_one = torch.abs(ct_plot[0])
        # heatmap_two = torch.max(heatmap_one, dim=0)[0]
        # heatmap_two = heatmap_two.cpu().detach().numpy()

        # my_index1 = str(my_index).zfill(5)
        # first_image = np.array(Image.open(args.data_dir+'/JPEGImages/480p/' + args.seq_name + '/' + my_index1 + '.jpg'))
        # original_shape = first_image.shape 

        # cam = heatmap_two / np.max(heatmap_two)
        # cam = cv2.resize(cam, (original_shape[1],original_shape[0]))
        # heatmap = cv2.applyColorMap(np.uint8(255*cam), cv2.COLORMAP_JET)
        # save_dir_res = os.path.join(args.seg_save_dir, 'heats', args.seq_name)
        # if not os.path.exists(save_dir_res):
        #     os.makedirs(save_dir_res)
        # if args.save_segimage:   
        #     seg_filename = os.path.join(save_dir_res, '{}.jpg'.format(my_index1))
        #     img2 = heatmap * 0.3 + first_image * 0.5
        #     cv2.imwrite(seg_filename, img2)

        # time.sleep(5)
        if x[0,0] == 0 or y[0,0] == 0:
            target_ct[0,0], target_ct[0,1] = (x[0,1]-1)*4, (y[0,1]-1)*4
        else:
            target_ct[0,0], target_ct[0,1] = (x[0,0]-1)*4, (y[0,0]-1)*4
        print(target_ct)
        # if my_index == 0:
        #     predict_ct = target_ct.cpu().numpy()
        #     ct_history.append((predict_ct[0,0], predict_ct[0,1]))
        # elif my_index == 1:
        #     print('ground truth ct: ', target_ct//4+1)
        #     print(x)
        #     print(y)
        #     bb
        #     predict_ct = ct_history[-1]
        #     x, y = (x-1)*4, (y-1)*4
        #     best_x, best_y = 0, 0
        #     distance = 10000000
        ...
        #         x_i, y_i = x[0,i], y[0,i]
        #         print(predict_ct)
        #         distance_o = torch.abs(x_i - predict_ct[0]) + torch.abs(y_i - predict_ct[1])
        #         if distance_o < distance:
        #             distance = distance_o
        #             best_x, best_y = x_i, y_i
        #     target_ct[0,0], target_ct[0,1] = best_x, best_y
        #     ct_history.append((target_ct.cpu().numpy()[0,0],target_ct.cpu().numpy()[0,1]))
        # else:
        #     predict_ct = motion_model_ct(ct_history, len(ct_history)+1)
        #     x, y = (x-1)*4, (y-1)*4
        #     best_x, best_y = 0, 0
        #     distance = 10000000
        #     for i in range(x.shape[1]):
        ...
        #output heat
        output = model(Variable(search, volatile=True).cuda(), Variable(previous, volatile=True).cuda(), Variable(target, volatile=True).cuda(), Variable(target_ct.float(), volatile=True).cuda(), Variable(previous_gauss, volatile=True).cuda())
        output1 = output[0].data[0,0].cpu().numpy()
     
        first_image = np.array(Image.open(args.data_dir+'/JPEGImages/480p/'+args.seq_name+'/00000.jpg'))
        original_shape = first_image.shape 
        ...
        #predict gauss
        previous_gauss = guassian(output1)
        previous_gauss = torch.from_numpy(previous_gauss).unsqueeze(0)
